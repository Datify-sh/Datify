...
impl ContainerProvider for RedisContainer {
    fn default_image(version: &str) -> String {
        format!("redis:{}-alpine", version)
    }
...
impl RedisContainer {
    pub async fn create(
        ...
        let container = docker
            ...
            .await
            .map_err(|e| AppError::Docker(format!("Failed to create Redis container: {}", e)))?;

        tracing::info!(
            "Created Redis container {} with ID {}",
            config.name,
            container.id
        );
